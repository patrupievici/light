...
// MUSCLE MAP WIDGET
// Usage:
//   MuscleMapWidget()           // standalone card (for profile)
//   MuscleMapWidget(expandVertically: true) // în [Expanded] — umple înălțimea (ex. ecran complet)
//   MuscleMapCard()             // compact card for home tab
// ─────────────────────────────────────────────────────────────────────────────
...
class MuscleMapWidget extends StatefulWidget {
  ...
  final bool compact;
  /// Dacă părintele oferă înălțime finită (ex. [Expanded] în [Column]), harta folosește tot spațiul deasupra legendei.
...
class _MuscleMapWidgetState extends State<MuscleMapWidget> {
  final _service = MuscleRecoveryService();
  Map<String, MuscleStatus> _muscles = {};
  bool _loading = true;
  late final PageController _pageController;
  int _pageIndex = 0;
  late final VoidCallback _onCacheRevision;

  // Colors — prototype orange scheme (ZVELT App.dc.html 182–211):
  // Recovering = accent orange, Ready = gold, Not trained = muted dark track.
  // SVG fills take string colors, so token Colors are serialized to hex.
  static const Color _readyGold = Color(0xFFE8A33D); // prototype #E8A33D
  static String get _colorUntrained => MuscleMapSvg.untrainedFill;
  static String get _colorRecovering => MuscleMapSvg.hexOf(ZveltTokens.brand);
  static String get _colorReady => MuscleMapSvg.hexOf(_readyGold);
  ...
  void initState() {
    ...
    MuscleRecoveryService.cacheRevision.addListener(_onCacheRevision);
  ...
  @override
  void dispose() {
    MuscleRecoveryService.cacheRevision.removeListener(_onCacheRevision);
  ...
  Future<void> _load() async {
    ...
    final status = await _service.getRecoveryStatus();
    if (!mounted) return;
    setState(() { _muscles = status; _loading = false; });
  }
  ...
  /// Accessibility: turns a muscle slug (e.g. `upper-back`) into a spoken name
  /// (e.g. `upper back`) using only the slug data already loaded.
  static String _humanizeSlug(String slug) => slug.replaceAll('-', ' ');

  /// Accessibility: a screen-reader summary of the body map for the given
  /// [side] ('Front'/'Back'), built entirely from the already-computed
  /// [_muscles] state (counts + per-muscle status labels). Invents nothing.
  String _semanticSummary(String side) {
    final ready = <String>[];
    final recovering = <String>[];
    for (final e in _muscles.entries) {
      switch (e.value.state) {
        case MuscleState.ready:
          ready.add(_humanizeSlug(e.key));
          break;
        case MuscleState.recovering:
          recovering.add('${_humanizeSlug(e.key)} (${e.value.label})');
          break;
        case MuscleState.untrained:
          break;
      }
    }
    final parts = <String>['$side body muscle map.'];
    if (ready.isEmpty && recovering.isEmpty) {
      parts.add('No muscles trained recently.');
    } else {
      if (ready.isNotEmpty) {
        parts.add('${ready.length} ready: ${ready.join(', ')}.');
      }
      if (recovering.isNotEmpty) {
        parts.add('${recovering.length} recovering: ${recovering.join(', ')}.');
      }
    }
    return parts.join(' ');
  }
  ...
  Widget build(BuildContext context) {
    if (_loading) {
      return LayoutBuilder(
        builder: (context, c) {
          ...
          final h = _mapHeight(context, c.maxWidth);
          return SizedBox(
            height: h + 52,
            child: const Center(
              child: CircularProgressIndicator(color: ZveltTokens.brand),
    ...
    final frontSvg = MuscleMapSvg._buildFrontSvg(colors);
    final backSvg  = MuscleMapSvg._buildBackSvg(colors);

    return LayoutBuilder(
      builder: (context, constraints) {
        final mapW = constraints.maxWidth;
        final flex = _useVerticalFlex(constraints);

        final pageView = PageView(
          controller: _pageController,
          onPageChanged: (i) => setState(() => _pageIndex = i),
          children: [
            // GestureDetector wrappers removed — `_handleTap` was an empty
            // stub waiting on SVG path hit-testing that's not on the
            // roadmap. Without a real action, the wrappers just consumed
            // taps that did nothing. PageView swipe still works on the
            // bare SvgPicture.
            if (flex)
              LayoutBuilder(
                builder: (context, lc) {
              ...
              Center(
                child: Semantics(
                  ...
                  child: SvgPicture.string(
                    ...
                    fit: BoxFit.contain,
                    width: mapW,
                    height: _mapHeight(context, mapW),
                  ),
                ),
              ),
            if (flex)
        ...
        return Column(
          ...
          children: [
            ...
            // Front/Back segmented chip toggle (prototype HTML 206; swipe on
            // the PageView still works as a bonus).
            ...
            Center(
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    _segment('Front', _pageIndex == 0, () => _goPage(0)),
                    const SizedBox(width: 6),
                    _segment('Back', _pageIndex == 1, () => _goPage(1)),
                  ],
                ),
              ),
            ),

            // Legend — Recovering (accent) / Ready (gold) / Not trained (txt3)
            const SizedBox(height: 12),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                const _LegendDot(
                    color: ZveltTokens.brand, label: 'Recovering'),
                const SizedBox(width: 14),
                const _LegendDot(color: _readyGold, label: 'Ready'),
                const SizedBox(width: 14),
                _LegendDot(color: ZveltTokens.text3, label: 'Not trained'),
              ],
            ),
          ],
        );
  ...
  void _goPage(int i) => _pageController.animateToPage(
        i,
        duration: const Duration(milliseconds: 220),
        curve: Curves.easeOutCubic,
      );

  Widget _segment(String label, bool active, VoidCallback onTap) => InkWell(
        onTap: onTap,
  ...
  bool _useVerticalFlex(BoxConstraints c) {
    return widget.expandVertically &&
        !widget.compact &&
        c.maxHeight.isFinite &&
        c.maxHeight > 120;
  }

  /// Înălțime fixă când nu folosim [expandVertically] (card home sau embed fără [Expanded]).
  double _mapHeight(BuildContext context, double maxWidth) {
    if (widget.compact) {
      return (maxWidth * 1.12).clamp(220.0, 300.0);
    }
    final h = MediaQuery.sizeOf(context).height;
    return (h * 0.62).clamp(380.0, 720.0);
  }
...
class MuscleMapCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Same surf-glass chrome as the sibling Home cards (prototype HTML 175):
    // surface gradient + 1px border + radius 24.
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Text('MUSCLE MAP', style: ZType.eyebrow.copyWith(fontSize: 12)),
              const Spacer(),
              InkWell(
                onTap: () => Navigator.of(context).push<void>(
                  MaterialPageRoute<void>(
                      builder: (_) => const MuscleMapScreen()),
                ),
          ...
          const MuscleMapWidget(compact: true),
        ],
      ),
    );
  }
...
class MuscleMapScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // Circle-back + title header — same idiom as every pushed screen
          // in the rebuild (CardioHistoryScreen / ConsistencyScreen).
          Padding(
          ...
          const Expanded(
            child: Padding(
              padding: EdgeInsets.fromLTRB(
                  ZveltTokens.s3, ZveltTokens.s1, ZveltTokens.s3, ZveltTokens.s2),
...
class MuscleMapSvg {
  ...
  static String _buildFrontSvg(Map<String, String> colors) {
    ...
    sb.write('<path d="M315.01 1025.17a.16.16 0 01.32.02c4.06 25.75 8.98 52.72 8.71 77.81q-.13 12.06-5.74 26.31c-7.2 18.3-8.93 38.57-15.95 56.93q-.18.48-.21-.03c-1.87-34.47-5.67-65.91-8.56-103.28q-.97-12.49 4.44-23.14 7.47-14.69 15.14-29.29c.81-1.55 1.35-3.62 1.85-5.33z"/>');
    ...
    sb.write('<path d="M412.77 1025.44a.14.14 0 01.27-.04c4.88 11.62 10.93 22.01 17.28 34.78 4.07 8.19 4.71 14.41 4.1 24.25-2.13 34.3-6.27 68.85-8.45 101.59q-.05.69-.31.05-1.48-3.67-2.28-6.75c-4.34-16.75-8.78-38.38-16.39-57.57q-1.4-3.55-2.2-10.11c-1.78-14.73-.2-31.24 2.04-45.88q3.06-20.02 5.94-40.32z"/>');
    sb.write('</g>');
    final cchest = colors['chest'] ?? untrainedFill;
    sb.write('<g fill="$cchest">');
    sb.write('<path d="M272.91 422.84c-18.95-17.19-22-57-12.64-78.79 5.57-12.99 26.54-24.37 39.97-25.87q20.36-2.26 37.02.75c9.74 1.76 16.13 15.64 18.41 25.04 3.99 16.48 3.23 31.38 1.67 48.06q-1.35 14.35-2.05 16.89c-6.52 23.5-38.08 29.23-58.28 24.53-9.12-2.12-17.24-4.38-24.1-10.61z"/>');
    sb.write('<path d="M416.04 435c-15.12.11-34.46-6.78-41.37-21.48q-1.88-3.99-2.84-12.18c-2.89-24.41-5.9-53.65 8.44-74.79 4.26-6.26 10.49-7.93 18.36-8.56q11.66-.92 23.32-.35c10.58.53 18.02 2.74 26.62 7.87 12.81 7.65 19.73 14.52 22.67 29.75 4.94 25.57.24 64.14-28.21 74.97q-12.26 4.67-26.99 4.77z"/>');
    sb.write('</g>');
    final cdeltoids = colors['deltoids'] ?? untrainedFill;
    sb.write('<g fill="$cdeltoids">');
    sb.write('<path d="M274.06 311.69q3.94 2.77 4.33 8.14.04.48-.38.73c-9.98 5.88-24.35 7.45-28.82 19.75-2.31 6.36-.97 17.35-1.43 23.68q-.55 7.51-5.73 14.07-10.37 13.11-13.81 16.67c-3.41 3.53-6.81 1.76-10.69-.47-15.42-8.87-24.95-25.45-22.52-43.22 2.05-14.92 12.71-25.79 24.06-35.02 16.99-13.82 35.58-17.99 54.99-4.33z"/>');
    sb.write('<path d="M450.39 320.75q-.95-.52-.7-1.58c1.57-6.61 5.8-9.1 12.14-11.9 24.99-11.03 43.76 3.33 60.17 20.74 20.73 21.99 11.81 56.44-14.82 68.19-4.41 1.94-6.79-1.03-9.81-4.51-5.81-6.7-13.46-14.12-15.99-22.8-3.93-13.43 4.32-27.54-9.64-37.62q-8.22-5.93-17.99-9.08-1.84-.59-3.36-1.44z"/>');
    sb.write('</g>');
    final cforearm = colors['forearm'] ?? untrainedFill;
    sb.write('<g fill="$cforearm">');
    ...
    sb.write('</g>');
    final cobliques = colors['obliques'] ?? untrainedFill;
    sb.write('<g fill="$cobliques">');
    sb.write('<path d="M264.21 435.53c-4.88-3.13-5.75-12.11-5.39-17.36q.03-.53.51-.75 1.8-.84 3.43.85 10.05 10.45 22.57 16.9c3.64 1.89 5.54 3.62 4.79 7.8q-.42 2.35-2.82 1.87-12.45-2.49-23.09-9.31z"/>');
    sb.write('<path d="M287.33 452.44c-4.05 4.46-10.38 11.38-16.28 14.3a.84.83 51.1 01-.9-.1c-6.29-5.17-12.54-18.97-14.21-25.09q-.91-3.34.85-8.81.12-.39.35-.05c2.41 3.65 4.59 7.74 8.67 9.76q10.18 5.05 21.27 9.01a.61.61 0 01.25.98z"/>');
    sb.write('<path d="M297.3 487.82c-7.36-4.23-16.68-11.37-20.55-17.57q-.32-.5.09-.92 8.72-9.04 19.84-17.87 1.46-1.17 2.81-1.67a.44.44 0 01.59.43c-.28 10.08-.4 20.42.65 30.43q.34 3.26-.68 6.15a1.9 1.9 0 01-2.75 1.02z"/>');
    sb.write('<path d="M257.35 456.18l13.68 16.63a1.86 1.82 22.9 01.4.95c.59 5.4-2.02 12.71-3.8 17.56q-.3.84-.84.13-11.85-15.55-9.77-35.17.04-.45.33-.1z"/>');
    sb.write('<path d="M271.69 494.07a1.53 1.52-61.8 01-.49-1.64l4.2-13.58a.98.98 0 011.51-.5c3.2 2.32 21.89 14.05 22.26 16.7q1.15 8.32.66 16.79a.9.9 0 01-1.34.73q-14.24-8.05-26.8-18.5z"/>');
    sb.write('<path d="M299.35 544.62c-7.52-6.03-16.15-13.43-24.23-21.24-6.93-6.7-6-17.19-4.88-26.06a.44.44 0 01.72-.28q13.31 11.88 28.41 21.38.43.27.6.75c2.33 6.49.95 18.37-.07 25.23q-.09.59-.55.22z"/>');
    sb.write('<path d="M299.09 575.53c-7.98-3.65-27.57-15.86-28.06-26.2q-.57-11.91.46-24.3a.36.36 0 01.67-.15q.84 1.36 2.17 2.54 10.59 9.45 21.68 18.31c4.37 3.49 4.34 6.46 4.16 11.74q-.3 8.82-.42 17.64-.01.72-.66.42z"/>');
    sb.write('<path d="M308.17 657.58c-7.39-.13-12.41-4.13-17.14-9.39q-11.86-13.22-23.92-26.37-.33-.36-.33-.85.09-23.18 1.81-46.22.53-7.13 2.49-14.41a.71.71 0 011.2-.3q11.54 12.06 25.82 21.1 3.36 2.12 3.62 5.17 2.06 23.67 3.86 47.36c.58 7.62 2.31 13.36 4.43 20.82q.47 1.66-.96 2.79-.39.31-.88.3z"/>');
    sb.write('<path d="M438.7 444.36c-2.09-4.03-.13-6.83 3.63-8.81 10.22-5.36 16.79-11 24.23-18.07a1.71 1.71 0 012.89 1.12c.33 4.74-.81 14.39-5.53 17.22-4.68 2.82-18.74 10.02-24.39 9.14q-.57-.09-.83-.6z"/>');
    sb.write('<path d="M457.39 466.73c-3.72-1.02-13.2-10.29-16.5-14.49a.52.52 0 01.24-.81q10.94-3.75 21.31-9c3.96-2.01 6.3-5.98 8.57-9.58q.38-.59.55.09c.82 3.33 1.54 6.17.38 9.58-2.55 7.44-7.62 18.79-13.66 24.01a.96.96 0 01-.89.2z"/>');
    sb.write('<path d="M428.43 487.22c-1.01-1.79-.82-4.55-.71-6.72q.78-15.08.48-30.27-.01-.59.55-.4 1.72.59 3.02 1.64 11.58 9.37 18.82 16.95c3.86 4.05-16.2 17.42-19.56 19.48a1.87 1.86 59.6 01-2.6-.68z"/>');
    sb.write('<path d="M470.76 456.28a.25.25 0 01.44.13q2.03 19.67-9.8 35.22-.37.48-.6-.08c-1.37-3.29-5.86-16.13-3.51-18.91q6.3-7.47 13.47-16.36z"/>');
    sb.write('<path d="M452.27 478.5c1.13.49 4.28 12.47 4.78 14.38q.14.5-.23.88-1.29 1.35-2.65 2.41-10.44 8.12-21.76 14.97-1.49.9-2.91 1.33a.81.81 0 01-1.05-.71q-.73-8.62.67-17.15.08-.47.44-.8c1.74-1.6 21.96-15.73 22.34-15.51a.58.03 31 00.37.2z"/>');
    sb.write('<path d="M428.22 519.14q.11-.36.43-.56 15.3-9.66 28.83-21.69a.43.42-22.6 01.71.29c.51 8.26 2.25 18.67-4.46 25.4q-11.8 11.84-25.03 22.09-.43.34-.49-.2c-.75-6.82-1.97-18.92.01-25.33z"/>');
    sb.write('<path d="M456.54 524.55a.04.04 0 01.07.02q1.52 13.67.41 27.4-.04.47-.28.88c-4.97 8.3-18.23 19.62-27.88 22.63q-.57.17-.58-.43-.05-10.31-.27-20.53-.1-4.8 2.63-7.09c8.54-7.13 18.56-14.62 25.9-22.88z"/>');
    sb.write('<path d="M418.89 657.11q-1.12-1.67-.43-3.63 3.27-9.38 4.04-18.23 1.97-22.81 3.58-45.65c.16-2.32.72-6.41 2.84-7.71q14.97-9.23 27.16-21.93.41-.42.71.08 1.29 2.15 1.53 4.2 3.23 27.74 3.13 56.8a1.3 1.28-24.5 01-.33.86q-12.74 13.93-25.55 27.75c-4.8 5.17-9.09 7.87-15.73 7.96q-.61.01-.95-.5z"/>');
    sb.write('</g>');
    final cquadriceps = colors['quadriceps'] ?? untrainedFill;
    sb.write('<g fill="$cquadriceps">');
    sb.write('<path d="M292.42 935.6q-.95-.52-1.57-1.4-4.1-5.79-7-13.53-7.8-20.79-13.3-42.33c-9.06-35.53-19.33-71.36-25.03-107.59-5.33-33.86 4-74.19 20.7-103.37q.35-.62.53.07c14.44 55.57 39.03 107.94 41.45 165.34 1.11 26.34.66 52.96-3.6 79.03-.63 3.83-4.73 27.81-12.18 23.78z"/>');
    ...
    sb.write('<path d="M322.69 945.72c-3.73 6.14-10.77-2.43-12.6-5.6-3.16-5.47-2.62-14.93-1.78-20.81 4.03-28.09 5.6-52.81 3.48-80.78q-.06-.79.28-.08 15.77 32.83 14.26 68.9c-.4 9.54-2.94 22.48-2.91 34.13q.01 3.02-.73 4.24z"/>');
    sb.write('<path d="M437.82 933.52c-8.9 14.18-15.15-26.74-15.46-29.25q-5.26-43.04-1.19-86.08c4.9-51.8 26.91-99.32 40.38-150.92q.18-.66.5-.06c17.25 31.67 25.39 68.28 20.54 104.36q-2.29 17.02-8.71 42.76-7.56 30.25-15.2 60.47-6.13 24.25-15.06 47.61-1.83 4.79-5.8 11.11z"/>');
    sb.write('<path d="M451.79 942.6c-9.95-10.01 4.97-42.91 8.94-55.41q12.55-39.53 19.27-80.47c.49-2.97 2.64-12.34 5.41-13.28a.83.83 0 011.09.64q.74 4 .45 7.92c-1.99 26.52-3.37 58.99-11.01 87.73q-2.53 9.5-7.46 18.8c-4.38 8.24-6.97 16.72-10.08 25.27q-1.66 4.54-4.55 8.63a1.35 1.35 0 01-2.06.17z"/>');
    sb.write('<path d="M406.69 946.81c-3.24-2.77-1.48-10.64-2.01-14.71q-2.23-17.18-2.57-22.16c-1.75-25.07 3.61-49.11 13.98-71.92q.23-.51.2.05c-1.2 19.15-1.28 38.18.83 57.38q1.68 15.4 3.39 30.8c.43 3.92-.31 9.71-2.09 13.33-1.62 3.28-7.58 10.77-11.73 7.23z"/>');
    sb.write('</g>');
    final ctrapezius = colors['trapezius'] ?? untrainedFill;
    sb.write('<g fill="$ctrapezius">');
    sb.write('<path d="M285.01 307.01a.89.89 0 01-.11-1.64q19.44-9.61 35.65-24.8 1.68-1.57 3.31-.31.4.32.45.82 1.25 12.61-1.57 25.41c-.74 3.32-2.55 4.23-5.9 4.48q-16.02 1.24-31.83-3.96z"/>');
    sb.write('<path d="M414 311.19c-5.24-.12-7.81-.64-8.9-6.27q-2.33-12.09-1.17-23.94.06-.61.61-.89 1.66-.85 3.65.99 16.12 14.87 33.97 23.63 3.65 1.79-.27 2.89-13.88 3.91-27.89 3.59z"/>');
    sb.write('</g>');
    final ctriceps = colors['triceps'] ?? untrainedFill;
    sb.write('<g fill="$ctriceps">');
  ...
  static String _buildBackSvg(Map<String, String> colors) {
    ...
    final cforearm = colors['forearm'] ?? untrainedFill;
    sb.write('<g fill="$cforearm">');
    sb.write('<path d="M878.44 534.38a.15.15 0 01.18-.13c.47.12 6.68 15.77 7.07 17.22q6.66 24.73 5.52 50.29c-.4 8.9-3.45 17.35-6.64 25.55-7.94 20.38-17.41 41.88-29.59 60.09a1.04 1.02-54.2 01-1.49.25c-.34-.26.37-1.45.47-1.83q5.58-20.8 8.97-42.08 8.65-54.15 15.51-109.36z"/>');
    sb.write('<path d="M893 518.93a.39.38 24.6 01.69-.25q5.97 7.83 13.11 15.27c8.08 8.4 1.41 28.73-5.88 37.12a1.05 1.05 0 01-1.63-.05c-6.09-7.93-5.41-18.74-4.97-28.44.36-8.12-.76-15.7-1.32-23.65z"/>');
    sb.write('<path d="M869.06 547.19c2.16.36 1.67 6.21 1.57 7.8q-2.54 38.84-9.11 77.16c-3.04 17.71-8.47 41.3-22.09 54.09a.38.38 0 01-.62-.41c14.51-40.44 19-84.26 26.8-126.31q.9-4.88 1.48-10.82.18-1.81 1.97-1.51z"/>');
    sb.write('<path d="M864.24 682.58q15.09-28.18 25.12-58.55c8.14-24.63 13.67-42.4 20.79-60.35q3.31-8.37 12.08-9.63c1.35-.2 3.68-.75 4.86.21q1.13.93.61 2.3-5.8 15.45-12.04 29.88c-5.79 13.39-14.92 28.68-20.32 40.14-6.12 13-28.07 59.18-31.64 56.64a.21.21 0 01.03-.36q.15-.07.34-.13.12-.04.17-.15z"/>');
    sb.write('<path d="M1272.99 519.43c.27-.33.33-.75.75-1.05a.32.32 0 01.5.29c-.7 7.22-1.77 14.33-1.66 21.54.13 8.94 2.13 24-5.35 31.17q-.37.35-.73 0c-7.63-7.55-14.2-28.29-6.52-36.92q6.6-7.41 13.01-15.03z"/>');
    sb.write('<path d="M1312.82 688.04c-4.78-6.01-7.2-10.8-11.76-19.56q-12.39-23.79-21.03-47.53c-4.86-13.36-5.22-26.17-3.83-40.19q1.13-11.5 2.69-19.53 2.72-13.98 9.59-26.79a.17.17 0 01.32.06q7.26 63.12 17.22 120.49 2.43 14.04 7.03 30.55c.22.79.74 1.33.36 2.4a.34.34 0 01-.59.1z"/>');
    sb.write('<path d="M1296.52 558.51c-.22-2.94-1.44-10.25 2-12.04a.62.61-18.4 01.89.44q6.25 35.69 12.21 71.07c3.88 23 8.77 46.2 16.73 68.19a.29.29 0 01-.47.31c-11.67-10.67-18.09-31.15-20.89-45.98q-7.27-38.55-10.47-81.99z"/>');
    sb.write('<path d="M1303.5 683.6c-2.89-.66-10.16-13.21-12.11-17.02-8.8-17.21-16.92-34.81-25.84-51.89-5.36-10.27-10.98-20.49-15.39-30.95q-5.86-13.86-11.07-27.8a1.63 1.62 79.5 011.5-2.2c13.02-.16 15.5 7.18 19.65 18.81q9.04 25.33 17.43 50.89 9.65 29.37 23.82 56.84.87 1.69 2.13 3.12.24.28-.12.2z"/>');
    sb.write('</g>');
    final cgluteal = colors['gluteal'] ?? untrainedFill;
    sb.write('<g fill="$cgluteal">');
    sb.write('<path d="M1045.06 626.19q1.42.61 4.11 4.4.27.39-.19.52c-14.47 4.12-26.13 7.4-38.13 15.77q-15.37 10.71-30.53 21.6a.55.54 74.9 01-.86-.5c1.19-13.13 10.35-35.23 20.46-45.06 9.14-8.88 34.99-1.11 45.14 3.27z"/>');
    ...
    sb.write('<path d="M1117.94 631.04q-.13-.03-.27-.06-.12-.02-.06-.13 2.58-4.2 7.05-5.92 12.71-4.87 26.13-5.81c12.93-.91 17.1 3.08 23.28 13.06 5.71 9.22 13.32 24.7 13.44 36.06q.01.76-.61.32-16.65-11.74-33.2-23.51c-10.03-7.14-23.72-10.58-35.76-14.01z"/>');
    ...
    sb.write('</g>');
    final chamstring = colors['hamstring'] ?? untrainedFill;
    sb.write('<g fill="$chamstring">');
    ...
    sb.write('<path d="M1030.2 791.53q.17-.36.38-.03c5.26 8.11 9.94 16.15 12.47 25.64 3.12 11.72 5.87 24.36 4.31 36.24q-.5 3.8-3.57 14.02c-10.75 35.81-12.83 74.2-18.5 111.1q-.82 5.4-2.55 10.55-.23.68-.59.07c-4.72-8.07-5.18-25.09-5.34-34.81-.7-43.69 1.92-87.82 6.38-131.28 1.41-13.74 1.99-21.15 7.01-31.5z"/>');
    ...
    sb.write('<path d="M1052.52 855.62a.04.04 0 01.08.01q1.07 9.9 2.17 19.87.33 3.04-2.37 14.18c-3.83 15.8-8.15 31.11-8.9 47.47-.99 21.61-3.11 45.66-9.92 66.3q-1.49 4.52-.87-.2 3.38-25.36 3.7-51.99c.05-3.74-.4-10.32.2-15.58 2.19-19.2 7.39-38.25 11.75-57.05 1.78-7.64 2.93-15.21 4.16-23.01z"/>');
    ...
    sb.write('<path d="M1202.61 741.08a.44.44 0 01.72.03c.52.82.9 1.86.95 2.91q.73 15.98.37 31.97-1.16 52.95-7.85 105.49-1.88 14.74-5.97 29.04-1 3.52-1.92 4.95-1.57 2.47-1.39-.37c.58-9.44 1.83-19.17 1.71-28.16-.32-24.52-4.94-49.11-3.95-72.75.69-16.54 2.5-33.51 7.54-49.38q2.99-9.4 6.61-18.6.74-1.88 3.18-5.13z"/>');
    sb.write('</g>');
    final clowerBack = colors['lower-back'] ?? untrainedFill;
    sb.write('<g fill="$clowerBack">');
    sb.write('<path d="M986.76 627.1c-3.13-13.13-7.31-49.77 7.27-58.07 2.4-1.37 4.8-.82 6.7 1.29 6.15 6.8 16.22 18.56 18.77 28.15a1.35 1.3 52.6 01-.11.98c-2.51 4.53-9.96 8.09-15.83 11.36q-5.47 3.06-11.33 10.52c-1.23 1.56-2.6 4.3-4.5 6.06a.59.58-28.2 01-.97-.29z"/>');
    sb.write('<path d="M1023.15 607.96a2.06 2.04-74.3 01-.94-1.69c-.17-10.98 5.04-24.58 8.79-34.9q15.61-42.83 36-83.59a1.11 1.1-62.5 011.51-.48c1.25.66 3.21 12.98 3.46 15.08q6.94 59.25 2.82 116.88-.62 8.66-3.1 19.37-.13.53-.59.24l-47.95-30.91z"/>');
    sb.write('<path d="M1090.76 581.75q.62-5.16 0-10.27.22-29.79 3.05-59.5 1.1-11.58 3.91-22.88.31-1.27.44-1.43 1.08-1.43 1.88.17 23.38 46.97 40.14 96.18c1.8 5.28 5.84 16.69 4.38 22.96a1.64 1.64 0 01-.71 1.01l-47.63 30.72q-1.12.72-1.34-.6-4.54-28-4.12-56.36z"/>');
    sb.write('<path d="M1151.19 603.31q-5.39-3.38-2.19-9.05 8.03-14.22 17.88-24.62c3.49-3.69 9.04.89 10.97 3.99q2.92 4.66 3.8 10.14 3.5 21.77-1.21 43.02a.96.96 0 01-1.77.28c-6.92-11.85-16.03-16.56-27.48-23.76z"/>');
    sb.write('</g>');
    final ctrapezius = colors['trapezius'] ?? untrainedFill;
    sb.write('<g fill="$ctrapezius">');
    ...
    final ctriceps = colors['triceps'] ?? untrainedFill;
    sb.write('<g fill="$ctriceps">');
    sb.write('<path d="M931.03 442.29c-2.01 2.57-6.52 9.71-10.12 9.17q-.52-.08-.8-.52-1.35-2.09-1.84-4.44c-2.25-10.87-3.28-22.88 1.35-33.38 5.45-12.33 18.27-23.68 29.61-31.2a.47.46 68.7 01.71.32l6.42 38.52q.09.54-.26.97c-.47.58-1.12 1.52-1.71 1.94q-9.11 6.58-18.08 13.36-2.9 2.2-5.28 5.26z"/>');
    ...
    sb.write('<path d="M903.57 519.67a1.84 1.82-5.4 01-1.12-.92q-3.54-6.97-3.68-15.19c-.37-21.2 3.8-42.53 9.5-63.44q.33-1.23.92-.1 4.64 8.78 8.6 18.67c2.88 7.21 4.19 12.98 1.88 20.57q-6.07 19.96-14.02 39.23-.65 1.58-2.08 1.18z"/>');
    sb.write('<path d="M1213.94 424.56q-2.02-1.5-3.08-3.02-.31-.46-.22-1 3.32-19.22 6.42-38.46.09-.56.56-.25 14.9 9.82 24.8 22.71c9.8 12.75 9.72 30.37 5.41 45.13a2.62 2.62 0 01-3.76 1.57c-3.26-1.77-6.22-6.71-8.62-9.67-5.24-6.46-14.75-12-21.51-17.01z"/>');
    ...
    sb.write('<path d="M1258.43 439.96q2.01 5.38 3.1 10.68c3.58 17.36 7.13 34.77 6.89 52.61q-.11 8.3-3.94 15.61a1.61 1.6 33.4 01-2.44.5c-1.45-1.19-1.9-3.58-2.43-4.94q-9.23-23.41-13.19-38.15c-2.63-9.81 6.82-27.63 11.53-36.35q.28-.5.48.04z"/>');
    sb.write('</g>');
    final cupperBack = colors['upper-back'] ?? untrainedFill;
    sb.write('<g fill="$cupperBack">');
    sb.write('<path d="M987.06 381.44c-8.48-5.06-14.14-13.28-18.82-22.92q-5.3-10.92-6.46-14.04c-1.49-4.01 35.14-19.22 39.61-20.97q2.75-1.08 4.33-.72c4.33.96 6.61 9.96 7.46 13.7q5.43 23.89 14.65 55.74.78 2.7-.88 4.39c-5.37 5.5-34.69-12.08-39.89-15.18z"/>');
    ...
    sb.write('<path d="M1017.71 404.73c-23.86 13.25-54.31 7.11-60.45-22.75-1.2-5.81-2.5-15.84.64-20.55 3.63-5.44 7.17 4.18 8.17 6.14 7.71 15.14 31.62 29.16 48.2 31.13q1.84.21 5.26 2.06.4.21.26.64-.86 2.65-2.08 3.33z"/>');
    sb.write('<path d="M1141.45 397.63a2.17 2.14-3.6 01-1.88-1.64q-.71-2.97.18-5.95 8.74-29.19 11.75-43.29c1.73-8.11 3.07-16.77 6.94-22.08 1.92-2.62 4.28-2.27 7.19-1.15q20.52 7.9 39.09 18.77a1.37 1.36 25.9 01.58 1.67c-6.05 15.46-12.98 30.84-28.43 39.45-9.45 5.26-25.83 15.17-35.42 14.22z"/>');
    sb.write('<path d="M1149.69 404.8q-2.04-1.15-2.45-3.5-.09-.53.41-.75c4.64-2.04 9.78-2.51 14.63-3.87 11.01-3.1 22.03-10.83 30.34-18.57q6.33-5.89 7.58-8.93c1.02-2.49 3.79-9.5 7-9.46q.52.01.87.39 2.71 3.01 2.81 7.2c.33 13.77-2.24 26.93-13.26 35.95-13.88 11.36-33.12 9.94-47.93 1.54z"/>');
